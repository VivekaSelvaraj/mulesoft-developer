...
app.post("/insert/data", async (req: any, res: any) => {
  try {
    ...
    if (reqBody.actor && reqBody.actress && reqBody.year && reqBody.director) {
      let queryForInserting: string = `insert into movies
...
app.get("/get/movies", async (req: any, res: any) => {
  try {
    ...
    if (actor) {
      let queryForgetting: string = `select * from movies where lead_actor='${actor}'`;
      ...
      let queryForgetting: string = `select * from movies`;
